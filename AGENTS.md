...
- **Automated QA = two planning suites:** **Live** — `run_planning_scene_acceptance.ps1` → `live_planning_scene_test.gd` (GdUnit + TestBattle, hidden window, **not** `--headless`). **Default headless gate** — `run_t3_mimic_headless.ps1` (fixture parity mirror of the live bible checklist via `run_planning_qa_gate.ps1`). Use `-LiveTier3` on the planning gate for live instead of headless.
