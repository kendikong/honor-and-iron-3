...
- **Class QA Knight bar (absolute):** Only **Knight** has owner QA **PASS** — all other classes **NOT PASS** until redone — [`docs/CLASS_QA_SIGNOFF.md`](docs/CLASS_QA_SIGNOFF.md) · `.cursor/rules/class-qa-knight-bar.mdc` · **All classes full QA mandatory (never stop / never ask):** `.cursor/rules/class-qa-all-classes-mandatory.mdc`
