...
- **Test execution hierarchy (absolute):** Never use test suites as interactive debuggers. Run only isolated single test cases (`GdUnitCmdTool.gd -t <test>`) during development. `run_all_background_class_tests.ps1` is heavy master CI only — **never run iteratively**; run only at final milestone completion or upon explicit user request.
