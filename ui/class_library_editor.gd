...
func _build_layout() -> void:
	...
	reload_btn.text = "Reload Factories"
	reload_btn.pressed.connect(_reload_factories)
	top_bar.add_child(reload_btn)
	var hint := Label.new()
	hint.text = "Edits apply live to DataLibrary (session). Save writes user:// overrides JSON."
	...
	d_scroll.add_child(_detail_vbox)
...
func _add_unit_button(parent: Control, unit: UnitData) -> void:
	...
	btn.pressed.connect(func() -> void: _select_unit(unit))
	parent.add_child(btn)
...
func _build_stats_section(unit: UnitData) -> void:
	...
	_bind_int(grid, "CON (×5 = Max HP)", unit.base_constitution, func(v: int) -> void: unit.base_constitution = v)
...
func _build_passives_section(unit: UnitData) -> void:
	...
	for passive: PassiveData in unit.passives:
		...
		var up_edit := _bind_multiline(box, "Upgraded Description", passive.upgraded_description, func(v: String) -> void:
		...
		_refresh_passive_preview(passive, preview)
		up_edit # silence unused


func _refresh_passive_preview(passive: PassiveData, preview: RichTextLabel) -> void:
	preview.text = ClassLibrarySchema.passive_bbcode(passive.description)
...
func _build_ability_row(ability: AbilityData) -> void:
	...
	impl_lbl.custom_minimum_size = Vector2(200, 120)
	col_impl.add_child(impl_lbl)
...
func _populate_ability_data_editor(parent: VBoxContainer, ability: AbilityData) -> void:
	...
	_bind_int(grid, "upgraded_range", ability.upgraded_range_tiles, func(v: int) -> void: ability.upgraded_range_tiles = v)
...
func _select_glossary() -> void:
	...
	for k: String in GameEnums.StatusType.keys():
		...
		var tooltip: String = CombatUiFormatters._status_name(st)
		var sys_text: String = ""
		for def_entry: Dictionary in ClassLibrarySchema.enum_definitions():
			if def_entry.get("category") == "StatusType" and def_entry.get("name") == k:
				sys_text = String(def_entry.get("system", ""))
				break
		_add_glossary_row(tooltip, tooltip, sys_text)
...
func _reload_factories() -> void:
	# DataLibrary caches statically; re-open scene to pick up code factory changes.
