...
## Fresh player units from code factories â€” no class_library_data.json overrides.
...
static func _archer(bow: WeaponData) -> UnitData:
	...
	var snipe := DataLibrary._make_ability(&"archer_snipe", "Snipe", 5, [DataLibrary._module(GameEnums.EffectType.DAMAGE, 3)], 1, GameEnums.StatType.PHYSICAL)
	var shove := DataLibrary._make_ability(&"archer_shove", "Repelling Shot", 2, [DataLibrary._module(GameEnums.EffectType.PUSH, 1)], 0)
...
static func _mage(staff: WeaponData) -> UnitData:
	...
	var fireball := DataLibrary._make_ability(&"mage_fireball", "Fireball", 3, [DataLibrary._module(GameEnums.EffectType.DAMAGE, 3)], 1, GameEnums.StatType.MAGICAL)
	var swap := DataLibrary._make_movement_ability(&"mage_swap", "Phase Swap", 2, [DataLibrary._module(GameEnums.EffectType.SWAP, 0)], 2)
...
static func _cleric(staff: WeaponData) -> UnitData:
	...
	var blessing := DataLibrary._make_ability(&"cleric_blessing", "Divine Shield", 2, [DataLibrary._module(GameEnums.EffectType.ARMOR_UP, 2)], 1)
	var pull := DataLibrary._make_movement_ability(&"cleric_pull", "Rescue Pull", 2, [DataLibrary._module(GameEnums.EffectType.PULL, 1)], 2)
...
static func _assassin(sword: WeaponData) -> UnitData:
	...
	var execute := DataLibrary._make_ability(&"assassin_execute", "Assassinate", 1, [DataLibrary._module(GameEnums.EffectType.DAMAGE, 3)], 1, GameEnums.StatType.PHYSICAL)
	var swap := DataLibrary._make_movement_ability(&"assassin_swap", "Shadow Swap", 1, [DataLibrary._module(GameEnums.EffectType.SWAP, 0)], 1)
...
static func _mercenary(sword: WeaponData) -> UnitData:
	...
	var rend := DataLibrary._make_ability(&"merc_rend", "Rend", 1, [DataLibrary._module(GameEnums.EffectType.DAMAGE, 2)], 1, GameEnums.StatType.PHYSICAL)
	var kick := DataLibrary._make_movement_ability(&"merc_kick", "Boot Kick", 1, [DataLibrary._module(GameEnums.EffectType.PUSH, 1)], 1)
...
static func _gryphon(lance: WeaponData) -> UnitData:
	...
	var swoop := DataLibrary._make_ability(&"gryphon_swoop", "Swoop Attack", 2, [DataLibrary._module(GameEnums.EffectType.DAMAGE, 2)], 1, GameEnums.StatType.PHYSICAL)
	var shove := DataLibrary._make_movement_ability(&"gryphon_shove", "Wing Buffet", 2, [DataLibrary._module(GameEnums.EffectType.PUSH, 1)], 2)
...
static func _monk(fist: WeaponData) -> UnitData:
	...
	var palm := DataLibrary._make_ability(&"monk_palm", "Palm Strike", 1, [DataLibrary._module(GameEnums.EffectType.DAMAGE, 2), DataLibrary._module(GameEnums.EffectType.PUSH, 1)], 1, GameEnums.StatType.PHYSICAL)
	var swap := DataLibrary._make_movement_ability(&"monk_swap", "Vault Swap", 1, [DataLibrary._module(GameEnums.EffectType.SWAP, 0)], 1)
...
static func _engineer(gun: WeaponData) -> UnitData:
	...
	var grenade := DataLibrary._make_ability(&"eng_grenade", "Grenade", 3, [DataLibrary._module(GameEnums.EffectType.RANGED_EXPLODE, 2)], 1)
	var pull := DataLibrary._make_ability(&"eng_pull", "Grappling Hook", 3, [DataLibrary._module(GameEnums.EffectType.PULL, 1)], 0)
...
static func _shaman(staff: WeaponData) -> UnitData:
	...
	var ward := DataLibrary._make_ability(&"shaman_ward", "Earth Ward", 3, [DataLibrary._module(GameEnums.EffectType.ARMOR_UP, 2)], 1)
	var shove := DataLibrary._make_ability(&"shaman_shove", "Gale Force", 2, [DataLibrary._module(GameEnums.EffectType.PUSH, 1)], 0)
...
static func _paladin(sword: WeaponData) -> UnitData:
	...
	var heal := DataLibrary._make_ability(&"paladin_heal", "Lay on Hands", 1, [DataLibrary._module(GameEnums.EffectType.HEAL, 2)], 1, GameEnums.StatType.MAGICAL)
	var swap := DataLibrary._make_movement_ability(&"paladin_swap", "Holy Swap", 2, [DataLibrary._module(GameEnums.EffectType.SWAP, 0)], 1)
