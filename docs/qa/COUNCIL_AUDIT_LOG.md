...
## 2026-09-04 — Council Round R139: Immediate Move Preview Clear On Walk Start & Elimination of Auto-Run Double Animation

### Scope
1. Fix duplicate autorun animation: in `presentation/combat_director.gd`, `_extract_commit_anim_events` and `_finalize_planning_commit_move_event` skip pseudo-move `UNIT_MOVED` events with `run_boost: true` generated by `AbilitySystem._apply_running_boost`. In `presentation/tactical_unit_layer.gd`, `_should_animate_planning_commit_move` ignores `run_boost: true` events.
2. Fix move preview clearing late: in `presentation/tactical_planning_overlay.gd`, `_queue_overlay_redraw` now calls `_queue_hover_tile_redraw()` and `_queue_static_tiles_redraw()` so floor layers (which draw the blue hover tile box and yellow move route follow line) immediately redraw when commit occurs. `_draw_hover_tile_on`, `_draw_hover_follow_route_on`, `_draw_interaction_overlay`, `_should_draw_interaction_overlay`, and `_display_move_route_cells` explicitly check `_should_draw_player_move_preview()`.
3. In `presentation/tactical_unit_layer.gd`, `_drain_planning_commit_queue` unsuppresses execution preview, requests overlay redraw, and resets `TacticalMapView` hover polling upon walk completion.
4. In `presentation/tactical_map_view.gd`, added `reset_hover_poll()` so hover polling refreshes smoothly once the walk completes even if the player hasn't moved their physical mouse.
5. In `tests/harness/action_range_regression_test.gd`, added regression assertions verifying exactly 1 `UNIT_MOVED` commit anim event for autorun and immediate clearing of overlay interaction elements.

### Council proof (pre-apply — BEFORE first production edit)
| Critic | Charter | Verdict | Rule / Exception IDs Cited | Focus & Proof |
|:---|:---|:---:|:---|:---|
| **1** | Bible paint & tiles | **PASS** | `EX-LOCKED-FIELD`, `MOVE_PREVIEW` | Move preview follow route and hover tile immediately unrender when the walk animation begins. `_hover_tile_layer` is queued on commit redraw. |
| **2** | Settle / bundle / commit | **PASS** | `COMMIT_TRUTH`, `INTENT_SSOT` | Filters out simulation pseudo-events (`run_boost: true`) so they are not promoted into duplicate physical move animations. Exactly one run animation plays. |
| **3** | Stand & range origins | **PASS** | `ACTION_RANGE_LATEST_STAND` | Action range paints from the latest committed stand; no regression to turn-start base board. |
| **4** | Global systems / anti-heuristic | **PASS** | `GLOBAL_SYSTEMS_FIRST`, `NO_BANDAID_FIXES` | Heuristics added: `none`. Fixes the canonical pipeline: event filtering in `CombatDirector` and complete layer invalidation in `TacticalPlanningOverlay`. |
| **5** | Perf & scheduling | **PASS** | `PERF_SCHEDULING` | Eliminates redundant tween execution and cleans up floor layer redraw invalidation. |
| **6** | QA-fix discipline | **PASS** | `QA_FIX_DISCIPLINE` | Automated regression test asserts single `UNIT_MOVED` event and immediate route unrendering. All 20 action range regression tests pass. |
| **7** | Class / skill | **PASS** | `CLASS_KIT_PARITY` | Uniformly fixes autorun and move previews for all classes without bespoke branching. |

**Verdict:** 7/7 PASS

### Changes applied
- `presentation/combat_director.gd`: Ignore `run_boost: true` pseudo-move events in `_extract_commit_anim_events` and `_finalize_planning_commit_move_event`.
- `presentation/tactical_unit_layer.gd`: Ignore `run_boost: true` events in `_should_animate_planning_commit_move`; unsuppress execution preview, redraw overlay, and reset hover polling when commit queue finishes draining.
- `presentation/tactical_planning_overlay.gd`: Ensure `_queue_overlay_redraw()` queues `_hover_tile_layer` and `_static_tiles_layer`; suppress hover tile and follow route when `_should_draw_player_move_preview()` is false.
- `presentation/tactical_map_view.gd`: Added `reset_hover_poll()`.
- `tests/harness/action_range_regression_test.gd`: Added assertions for single autorun `UNIT_MOVED` event and immediate route clearance.
