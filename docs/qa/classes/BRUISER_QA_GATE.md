...
**Scope:** **Class validation** — every Bruiser **active skill**, **movement skill**, and **passive** in `core/factory/classes/bruiser_factory.gd` behaves per `class_abilities.txt` § Bruiser. **Not** gameplay-core planning/UI (see [`PLANNING_QA_GATE.md`](../planning/PLANNING_QA_GATE.md)).
...
**P3 clone authority:** [`docs/design/knight-template.md`](../../design/knight-template.md) · [`docs/qa/classes/KNIGHT_QA_GATE.md`](KNIGHT_QA_GATE.md) · [`CLASS_QA_SIGNOFF.md`](../CLASS_QA_SIGNOFF.md)
...
| **Manual** | `docs/qa/planning/PLANNING_SKILL_QA_CHECKLIST.md` per ability | Required for feel/pixels Tier 1 cannot see |
...
| **Violent Collision:** DASH 3 + BULLDOZE; on hit DASH 5 + BULLDOZE same line | single `MOVE` | single `DASH` + BULLDOZE + `IF_LINE_COLLISION` → `MODIFY_PRIMARY_RANGE 5` layer (no second module, no `violent_collision_recast` knob) |
...
ARTIFACT: docs/qa/classes/BRUISER_QA_GATE.md, qa_bruiser_gate_canonical.txt, docs/qa/manifests/bruiser_meta_critic_manifest.json
```
