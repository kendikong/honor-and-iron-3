...
**Scope:** Class validation — every Archer **active skill**, **movement skill**, and **passive** in `core/factory/classes/archer_factory.gd` per `class_abilities.txt` § Archer. **Not** gameplay-core planning/UI (see [`PLANNING_QA_GATE.md`](../planning/PLANNING_QA_GATE.md)).
...
| **Manual** | `docs/qa/planning/PLANNING_SKILL_QA_CHECKLIST.md` | Owner still required for feel until matrix `PASS` |

---

## Meta-critic

Same contract as [`KNIGHT_QA_GATE.md`](KNIGHT_QA_GATE.md) § Meta-critic. **Manifest:** `docs/qa/manifests/archer_meta_critic_manifest.json` — gate fails if matrix PASS exceeds manifest.
