...
```text
══════════════════════════════════════
GAUNTLET SCORE │ [piece-id] │ Critic pass Cn │ not self-grade (subagent)
SCORE: x/100 │ THRESHOLD: y │ PASS|FAIL │ CLIMBING|STALLED|SLIPPED
DELTA: +N vs C(n-1) (was z)
══════════════════════════════════════
```

If the lead posts a score **without** spawning the critic subagent, the GAUNTLET SCORE line must say **`self-grade (INVALID)`** — that score cannot satisfy PASS or LOCK (Rule 4).
