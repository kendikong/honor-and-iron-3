...
static func build(basic_axe: WeaponData) -> UnitData:
	...
	var push_through := DataLibrary._make_movement_ability(
		...
		"Push Through",
		1,
		[DataLibrary._module(GameEnums.EffectType.MOVE_INTO_AND_PUSH, 1)],
		2,
	...
	var suplex = DataLibrary._make_ability(&"bruiser_suplex", "Suplex", 1, [
		...
		DataLibrary._module(GameEnums.EffectType.THROW_BEHIND, 0)
	], 1, GameEnums.StatType.PHYSICAL)
	...
	var earthshatter = DataLibrary._make_ability(&"bruiser_earthshatter", "Earthshatter", 1, [
		DataLibrary._module(GameEnums.EffectType.DAMAGE, 2),
		DataLibrary._module(GameEnums.EffectType.DESTROY_OBSTACLE, 0)
	], 1, GameEnums.StatType.PHYSICAL, GameEnums.TargetShape.ARC, 1)
	...
	var guttural_roar = DataLibrary._make_ability(&"bruiser_guttural_roar", "Guttural Roar", 0, [
		DataLibrary._module(GameEnums.EffectType.PUSH, 1),
		DataLibrary._status_module(GameEnums.StatusType.STAT_DEBUFF_DEF, 1, 2)
	...
	var headbutt = DataLibrary._make_ability(&"bruiser_headbutt", "Headbutt", 1, [
		DataLibrary._module(GameEnums.EffectType.DAMAGE, 3),
		DataLibrary._module(GameEnums.EffectType.DAMAGE_SELF, 1),
		DataLibrary._status_module(GameEnums.StatusType.STAGGER, 1),
		DataLibrary._status_module_self(GameEnums.StatusType.STAGGER, 1)
	], 1, GameEnums.StatType.PHYSICAL)
	...
	## Authored gate (ability-data.md Â§2.7) â€” not anonymous modifiers.
	...
	var belly_flop = DataLibrary._make_ability(&"bruiser_belly_flop", "Belly Flop", 2, [
		DataLibrary._module(GameEnums.EffectType.TELEPORT_CASTER, 0),
		DataLibrary._module(GameEnums.EffectType.DAMAGE, 2)
	...
	belly_flop.upgraded_modules.append(DataLibrary._module(GameEnums.EffectType.PUSH, 1))
	belly_flop.upgraded_modules[2].legacy_modifiers["belly_flop_push"] = 1
