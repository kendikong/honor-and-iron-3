...
static func _dismantle() -> AbilityData:
	...
	base.target_def_pct_loss = 0.25
	var upgraded := _clone([base])
	upgraded[0].on_hit_scrap = 1
...
static func _sludge_bomb() -> AbilityData:
	...
	var oil_layer := _layer(oil)
	oil_layer.terrain_id = &"oil"
	oil_layer.hazard_duration = 3
	oil_layer.oil_field = true
	base.layers = [oil_layer]
	var upgraded := _clone([base])
	upgraded[0].ignite_oil_area = true
...
static func _construct_turret() -> AbilityData:
	...
	base.construct_spawn = true
	base.turret_attack = 1
	var upgraded := _clone([base])
	upgraded[0].on_death_adjacent_damage = 2
...
static func _frag_bomb() -> AbilityData:
	...
	base.ignite_oil = true
	var upgraded := _clone([base])
	upgraded[0].construct_destruction_refund_ap = 1
...
static func _magnetic_mine() -> AbilityData:
	...
	base.mine_pull = 2
	base.mine_damage = 2
	base.mine_explode = true
	var upgraded := _clone([base])
	upgraded[0].absorbs_items_scrap = true
...
static func _tesla_barricade() -> AbilityData:
	...
	base.construct_spawn = true
	base.tesla_wall = true
	var upgraded := _clone([base])
	upgraded[0].manual_detonation_stagger = true
...
static func _flak_cannon() -> AbilityData:
	...
	var upgraded := _clone([base])
	upgraded[0].scrap_attack_bonus = 2
	upgraded[0].scrap_bleed_weapon = true
...
static func _wrench_smack() -> AbilityData:
	...
	base.wrench_smack = true
	var upgraded := _clone([base])
	upgraded[0].wrench_strength_bonus = 1
...
static func _emp_grenade() -> AbilityData:
	...
	base.emp_grenade = true
	base.mechanical_boss_damage_wpn = 3
	var upgraded := _clone([base])
	upgraded[0].emp_friendly_construct_heal = 2
	upgraded[0].emp_friendly_construct_overclock = true
...
static func _rocket_launcher() -> AbilityData:
	...
	base.rocket_launcher = true
	base.destroy_terrain = true
	base.exhaust_next_turn = true
	base.delayed_next_turn = true
	var upgraded := _clone([base])
	upgraded[0].sacrifice_construct_instant = true
	...
	sacrifice.sacrifice_construct_instant = true
...
static func _scrap_shield() -> AbilityData:
	...
	base.scrap_shield = true
	base.scrap_multiplier = 2
	var upgraded := _clone([base])
	upgraded[0].shield_depletion_explode = true
...
static func _manual_detonation() -> AbilityData:
	...
	base.manual_detonation = true
	base.does_not_consume_action_slot = true
	base.limit_once_per_turn = true
	base.set_condition_occupant(GameEnums.ModuleTargetFilterOccupant.ALLY_CONSTRUCT)
	var upgraded := _clone([base])
	upgraded[0].refund_scrap = 1
...
static func _overdrive_injection() -> AbilityData:
	...
	base.overdrive_injection = true
	base.construct_unmitigated_damage = 2
	base.set_condition_occupant(GameEnums.ModuleTargetFilterOccupant.ALLY_CONSTRUCT)
	var upgraded := _clone([base])
	upgraded[0].refund_scrap_on_construct_death = 1
...
static func _barbed_wire() -> AbilityData:
	...
	base.terrain_id = &"barbed_wire"
	base.hazard_duration = 2
	base.barbed_wire = true
	base.next_attack_bleed_weapon = true
	base.entry_root = true
	var upgraded := _clone([base])
	upgraded[0].adjacent_defense_bonus = 1
