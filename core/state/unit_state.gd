...
func reset_for_turn() -> void:
	...
	passive_flags.erase("if_collided_recast_used")
	...
	passive_flags.erase("pull_immune_this_turn")
	passive_flags.erase("slow_immune_this_turn")
	passive_flags.erase("cannot_gain_shield")
...
func get_ability_range(ability_data: AbilityData, ignore_steady_aim: bool = false) -> int:
	...
	var authored_range: int = (
		...
		if not active_modules.is_empty()
		else ability_data.get_active_card_range(is_ability_upgraded(ability_data.id))
	)
	if (
		ability_data != null
		and (
