...
func update_drag(local: Vector2) -> void:
	...
	if dragging and force_basic_movement:
		_clamp_postmove_drag_for_forbidden_hover(_drag_unit_id)
	if _movement_route_paint_allowed() and cell_changed:
		if not _basic_postmove_corridor_preview_overrides_drag(_drag_unit_id):
	...
	if cell_changed:
		...
		var postmove_actor: UnitState = _proj_unit(_drag_unit_id)
		if postmove_actor != null and _postmove_live_corridor_active(postmove_actor):
			_refresh_movement_slot_hover_preview(postmove_actor, cell)
	if cell_changed:
...
func _refresh_drag_preview_now() -> void:
	...
	if route_painted and force_basic_movement and not preview_waypoints.is_empty():
	...
	if route_painted:
		...
		_sync_drag_route_stand()
	elif drag_actor != null and _postmove_live_corridor_active(drag_actor):
		_refresh_movement_slot_hover_preview(drag_actor, cell)
	elif (
		force_basic_movement
...
func _clamp_postmove_drag_for_forbidden_hover(unit_id: int) -> void:
	if not dragging or not force_basic_movement or _director == null or unit_id < 0:
		return
	var actor: UnitState = _proj_unit(unit_id)
	if actor == null:
		return
...
func _authoritative_move_hover_paths_payload() -> Dictionary:
	...
	if path.is_empty():
		return {}
	if path.size() >= 2 and force_basic_movement:
...
func _anchor_preview_path_for_active_move_leg(unit_id: int, start_board: BoardState) -> void:
	...
	if actor == null:
		return
	if _postmove_live_corridor_active(actor):
		return
	var stand: Vector2i = Vector2i(-999999, -999999)
	var ability: AbilityData = _selected_ability_data(actor)
	## MOVE module legs use the same painted-route path as premove — do not collapse here.
...
func on_hover_moved(cell: Vector2i) -> void:
		...
		if p_unit != null:
			...
			if _post_move_basic_planning_open(p_unit):
				...
				if (
					...
					_clear_hover_drag_route()
				if not dragging and not _postmove_live_corridor_active(p_unit):
			...
			var postmove_live_corridor: bool = _postmove_live_corridor_active(p_unit)
			...
			if allow_hover_paint and not postmove_live_corridor:
	...
	if (
		...
		var corridor_unit: UnitState = _proj_unit(_director.selected_unit_id)
		if corridor_unit != null and _postmove_live_corridor_active(corridor_unit):
...
func _sync_movement_preview_after_hover_sim(cell: Vector2i) -> void:
	...
	if hover_unit == null:
		return
	if dragging and force_basic_movement and _post_move_basic_planning_open(hover_unit):
		...
		if leg_origin.x > -900000:
			...
			if (
				...
				return
	if dragging and not _postmove_live_corridor_active(hover_unit):
		return
...
func _refresh_selected_interaction_preview() -> void:
	...
	if _selection_hover_corridor_paint_active():
		var corridor_unit: UnitState = _proj_unit(_director.selected_unit_id)
		if corridor_unit != null and _movement_slot_hover_preview_applies(corridor_unit, cell):
			_refresh_movement_slot_hover_preview(corridor_unit, cell)
			_refresh_click_target_highlight()
			return
	_restore_hover_preview()
...
func _restore_hover_preview() -> void:
	...
	var preserve_painted_route: bool = (
		not dragging
		and _drag_route.size() >= 2
		and _selection_hover_corridor_paint_active()
	)
...
func _restore_locked_painted_preview_paths(unit_id: int) -> void:
	if unit_id < 0:
		return
	var actor: UnitState = _proj_unit(unit_id)
	if actor != null and _post_move_basic_planning_open(actor) and not _frozen_painted_leg_routes.has(unit_id):
		_seal_postmove_painted_drag_buffer_if_needed(actor)
	...
	if _skill_interaction_active() or aiming:
		...
		if hover_unit.is_enemy():
			...
			if ability == null and _director != null:
				var awaiting_pick: TimelineAction = _director.find_awaiting_action(p_unit.id)
				if awaiting_pick != null:
					ability = awaiting_pick.ability
			if ability != null and _director != null:
	...
	if _director != null and _director.selected_unit_id >= 0:
		...
		if p_unit != null:
			...
			if ability != null and _is_awaiting_movement_endpoint(p_unit, ability):
				return false
			if _postmove_live_corridor_active(p_unit):
				return false
...
func active_movement_planning_step(p_unit: UnitState) -> bool:
	if _director == null or p_unit == null:
		return false
	if _director.unit_has_wait_planned(p_unit.id):
		return false
...
func _basic_painted_drag_orbit_guard_active() -> bool:
	if not dragging or not force_basic_movement:
		return false
	if _director == null or _director.selected_ability_index >= 0:
		return false
...
func _sanitize_drag_route_context() -> void:
	...
	if (
		dragging
		and force_basic_movement
		and _painted_drag_route_matches_leg(unit)
	):
		_trim_drag_route_forbidden()
		_sync_drag_route_stand()
		return
	if not MovementSystem._is_legal_walk(board, origin, waypoints, budget, move_cost, unit, ability):
		var safe_path: Array[Vector2i] = MovementSystem.find_path(
			board, origin, final_cell, budget, mt, move_cost, ability,
		)
...
func _sync_drag_route_stand() -> void:
	...
	if _drag_route.size() >= 2:
		...
		var post_actor: UnitState = _proj_unit(_drag_unit_id)
		var hover_cell: Vector2i = _pointer_grid_cell()
		if (
			post_actor != null
			and _post_move_basic_planning_open(post_actor)
			and _director.board != null
			and _director.board.is_in_bounds(hover_cell)
		):
			_refresh_movement_slot_hover_preview(post_actor, hover_cell)
	else:
		_clear_stale_painted_preview_route(_drag_unit_id)


## Modular skill landing post-move: corridor per hover, never accumulated drag paint.
func _modular_postmove_corridor_active(p_unit: UnitState) -> bool:
	if p_unit == null or _director == null or not force_basic_movement:
		return false
	if not _post_move_basic_planning_open(p_unit):
		return false
	var awaiting: TimelineAction = _director.find_awaiting_action(p_unit.id)
	return awaiting != null and awaiting.ability != null


## Per-hover corridor after skill landing — not sealed painted post-move legs.
func _postmove_live_corridor_active(p_unit: UnitState) -> bool:
	if p_unit == null or _director == null or not force_basic_movement:
		return false
	if not _post_move_basic_planning_open(p_unit):
		return false
...
func _basic_postmove_corridor_preview_overrides_drag(unit_id: int) -> bool:
	...
	if actor == null:
		return false
	return _postmove_live_corridor_active(actor)
...
func _movement_slot_hover_preview_applies(p_unit: UnitState, cell: Vector2i) -> bool:
	if _director == null or not _director.board.is_in_bounds(cell):
		return false
	if _postmove_live_corridor_active(p_unit) and _attack_target_id_at_cell(p_unit, cell) < 0:
		return true
	if painted_move_route_locked(p_unit):
		return false
...
func _write_movement_hover_preview_paths(
	...
	if (
		...
		if (
			...
			return
	if actor != null and _postmove_live_corridor_active(actor):
		var post_origin: Vector2i = _active_move_drag_origin(actor)
		if post_origin.x <= -900000:
			return
		var forbidden: Dictionary = CombatPlanningPreview.prior_leg_forbidden_cells(
			_director, unit_id, post_origin,
		)
		if forbidden.has(hover_cell) and hover_cell != post_origin and not _can_move_to(actor, hover_cell):
	...
	if existing.size() >= 2:
		...
		if actor != null and (
			live_move_hover_rewrite_applies(actor, hover_cell)
			or _postmove_live_corridor_active(actor)
		):
			pass
		else:
			return
...
func _route_waypoints_for_commit() -> Array[Vector2i]:
	if _drag_route.size() >= 2:
		...
		return raw
	var unit_id: int = _drag_unit_id if dragging else (
	...
	if unit_id >= 0:
		...
		if painted_path.size() >= 2:
			var from_preview: Array[Vector2i] = []
			for i: int in range(1, painted_path.size()):
				from_preview.append(painted_path[i] as Vector2i)
			return from_preview
	return []
...
func _corridor_waypoints_to_cell(actor: UnitState, cell: Vector2i) -> Array[Vector2i]:
	...
	return CombatPlanningPreview.corridor_waypoints_to_cell(
		...
		origin,
		cell,
		_drag_max_steps(actor),
		_route_pathfinding_ability(actor),
		_director,
		actor.id,
	)
...
func _build_commit_slots_at_cell(
	...
	if ability_index >= 0 and ability != null and not force_basic_movement:
		...
		if (
			...
			effective_waypoints = _corridor_waypoints_to_cell(actor, cell)
		if (
			effective_waypoints.is_empty()
	...
	if (
		...
		if move_waypoints.is_empty() and _drag_route_commits_active():
			move_waypoints = _route_waypoints_for_commit()
...
func _final_commit_slots_for_drop_at_cell(
	...
	if dropped_on != null and dropped_on.id != unit_id:
		if _is_selectable_player_unit(dropped_on):
			var actor: UnitState = _proj_unit(unit_id)
			if actor == null or force_basic_movement or _director.selected_ability_index < 0:
				return _empty_commit_slots()
			var params: Dictionary = _commit_interaction_params(cell, dropped_on.id)
			return _slots_with_facing_for_commit(
				unit_id,
