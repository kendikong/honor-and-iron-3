...
func _authoritative_route_for_unit(unit_id: int) -> Array:
	var frozen_geometry: Array = CombatPlanningPreview.sealed_painted_geometry_route(
		preview_state, unit_id,
	)
	if frozen_geometry.size() >= 2:
	...
	if receipt != null and receipt.unit_id == unit_id:
		var receipt_fresh: bool = (
			_intent_state == null
			or _intent_state.hover_coord.x <= -900000
			or receipt.hover_cell == _intent_state.hover_coord
		)
		if receipt_fresh:
...
func _end_drag_interaction(restore_committed: bool, snap_back: bool = false) -> void:
	...
	var froze_paint_geometry: bool = false
	if finishing_drag_unit_id >= 0 and not snap_back:
		...
		if (
			...
			CombatPlanningPreview.freeze_painted_geometry_route(
				preview_state, finishing_drag_unit_id, _drag_route,
			)
			froze_paint_geometry = true
	_invalidate_planning_hover_cache()
	_clear_drag_preview_cache()
	if finishing_drag_unit_id >= 0:
		if finishing_drag_unit_id != sealed_unit_id and not froze_paint_geometry:
...
func on_hover_moved(cell: Vector2i) -> void:
	...
	if planning_cell_changed:
		...
		if (
			...
			var orbit_unit: UnitState = _proj_unit(_director.selected_unit_id)
			if (
				orbit_unit != null
				and _voluntary_walk_orbit_settle_open(orbit_unit)
				and not dragging
			):
				if (
					...
					_settled_hover_preview = null
				if not preview_state.is_painted_leg_sealed(orbit_unit.id):
					CombatPlanningPreview.clear_unit_preview_path(
						preview_state, orbit_unit.id,
					)
					_sync_movement_hover_paths_to_overlay(orbit_unit.id)
...
func _movement_preview_resync_after_sim_allowed(p_unit: UnitState) -> bool:
	...
	if _painted_receipt_orbit_extend_active(p_unit, hover_cell):
		if _armed_awaiting_move_orbit_settle_open(p_unit):
			return true
		return false
...
func _flush_hover_heavy_sync() -> void:
	...
	if not dragging and _director != null and _director.selected_unit_id >= 0:
		...
		if (
			...
			and not _painted_receipt_orbit_extend_active(flush_unit, flush_cell)
		):
...
func _run_hover_sim_refresh() -> void:
	...
	if _director.selected_unit_id >= 0:
		var sim_actor: UnitState = _proj_unit(_director.selected_unit_id)
		var settle_fresh_skip: bool = _hover_settle_fresh_at(_director.selected_unit_id, cell)
		if (
			settle_fresh_skip
			and sim_actor != null
			and _armed_painted_orbit_hover_parity_active(sim_actor, cell)
		):
			settle_fresh_skip = false
		if settle_fresh_skip:
			_last_sim_hover_refresh_cell = cell
			if not dragging:
				refresh_mouse_cursor(cell)
			return
...
func _refresh_hover_interaction_preview(cell: Vector2i) -> void:
	...
	if not p_unit.active_abilities.is_empty() and _director.selected_ability_index >= 0:
		...
		if (
			armed_ability != null
			and _armed_painted_receipt_orbit_extend_active(p_unit, cell)
		):
			_refresh_voluntary_walk_hover_preview(p_unit, cell)
			_refresh_click_target_highlight()
			return
	...
	if _director.selected_ability_index >= 0 and action_range_visible_for_hover():
		...
		if (
			...
			and _is_awaiting_movement_endpoint(p_unit, stand_only_ability)
			and _voluntary_walk_orbit_settle_open(p_unit)
			and not dragging
			...
			var orbit_stand_res: Dictionary = _preview_at_interaction_cell(
				p_unit.id, cell, cell, -1, [], _snapshot_drag_legal_move_tiles(),
			)
			if not _is_invalid_dict(orbit_stand_res):
				_apply_settled_preview_result(orbit_stand_res)
			_refresh_click_target_highlight()
			return
		if (
...
func _is_hover_move_cell(p_unit: UnitState, cell: Vector2i) -> bool:
	...
	if _awaiting_target_pick_blocks_premove():
		return false
	if (
		_painted_preview_route_active(p_unit.id)
...
func _clear_hover_drag_route() -> void:
	if _drag_route.is_empty() and _drag_unit_id < 0:
		return
	if _drag_unit_id >= 0 and _director != null:
		var clear_actor: UnitState = _proj_unit(_drag_unit_id)
		if (
			clear_actor != null
			and awaiting_targeting_active()
			and _awaiting_painted_drag_matches_leg(clear_actor)
			and not preview_state.is_painted_leg_sealed(clear_actor.id)
		):
			_restore_locked_painted_preview_paths(clear_actor.id)
	if _drag_route.is_empty() and _drag_unit_id < 0:
		return
	_drag_route.clear()
	_drag_unit_id = -1
...
func _sync_sealed_preview_to_overlay(unit_id: int) -> void:
	if unit_id < 0 or _planning == null:
		return
...
func _armed_awaiting_move_orbit_settle_open(p_unit: UnitState) -> bool:
	...
	if ability == null:
		ability = _selected_ability_data(p_unit)
	return ability != null and _is_awaiting_movement_endpoint(p_unit, ability)


func _voluntary_walk_orbit_settle_open(p_unit: UnitState) -> bool:
	if p_unit == null:
		return false
	return (
		_voluntary_walk_orbit_phase_open(p_unit)
		or _armed_awaiting_move_orbit_settle_open(p_unit)
	)
...
func _seal_painted_preview_landing_if_needed(p_unit: UnitState) -> void:
	...
	_sync_sealed_preview_to_overlay(p_unit.id)
...
func _do_refresh_live_interaction_preview(
	...
	if res.is_empty():
		res = _preview_at_interaction_cell(
			unit_id, cell, move_coord, attack_target_id, effective_waypoints, _snapshot_drag_legal_move_tiles(),
		)
...
func _store_intent_snapshot(
	...
	if unit_id >= 0:
		...
		elif sealed_route is Array and (sealed_route as Array).size() == 1:
			CombatPlanningPreview.set_unit_stand_anchor_path(
				preview_state, unit_id, sealed_route[0] as Vector2i,
			)
...
func _preview_paths_snapshot_for_settle(
	...
	if (
		...
		if receipt_orbit.size() < 2:
			receipt_orbit = _assemble_voluntary_walk_preview_path(
				unit_id, settle_actor, _hover_cell, [], true,
			)
		if receipt_orbit.size() >= 2:
	...
	if (
		...
		var unreachable_stand: Vector2i = _settle_phase_entry_stand(settle_actor)
		if unreachable_stand.x > -900000:
			snapshot[unit_id] = [unreachable_stand]
			return snapshot
	if (
		settle_actor != null
		and _voluntary_walk_orbit_settle_open(settle_actor)
		...
		var orbit_waypoints: Array[Vector2i] = settle_waypoints
		var orbit_parity: bool = _painted_receipt_orbit_extend_active(
			settle_actor, _hover_cell,
		)
	...
	if (
		...
		snapshot[unit_id] = [move_origin_settle]
		return snapshot
...
func painted_move_route_locked(
	...
	if hover_cell.x > -900000 and _armed_awaiting_move_orbit_settle_open(p_unit):
		...
		if (
			_painted_preview_route_active(p_unit.id)
			and _planning != null
			and _planning.is_hover_move_tile(hover_cell)
		):
			return false
...
func _hover_extends_walk_geometry(p_unit: UnitState, cell: Vector2i) -> bool:
	...
	if (
		_painted_preview_route_active(p_unit.id)
		and active_movement_planning_step(p_unit)
		and _planning != null
...
func movement_hover_route_display_applies(actor: UnitState, ability: AbilityData) -> bool:
	...
	if (
		...
		and not AbilitySystem.motion_requires_occupied_target(actor, ability)
	):
		return true
	return false
...
func _armed_painted_receipt_orbit_extend_active(p_unit: UnitState, cell: Vector2i) -> bool:
...
## Armed trample awaiting MOVE: orbit cells around frozen painted geometry (premove parity).
func _armed_painted_orbit_hover_parity_active(p_unit: UnitState, cell: Vector2i) -> bool:
	if p_unit == null or _director == null or dragging:
		return false
	if _director.selected_ability_index < 0:
		return false
	if not preview_state.is_painted_leg_sealed(p_unit.id):
...
## Painted leg seal state for assembler — actual preview seal flag only.
...
func _restore_parked_preview_board(parked_preview_board: BoardState) -> void:
	if parked_preview_board != null and preview_state.preview_board == null:
...
func _do_refresh_voluntary_walk_hover_preview(p_unit: UnitState, cell: Vector2i) -> void:
	...
	if (
		...
		if sealed_orbit_extend:
			...
			var orbit_parity_probe: bool = _painted_receipt_orbit_extend_active(p_unit, cell)
			probe_path = _assemble_voluntary_walk_preview_path(
				p_unit.id, p_unit, cell, settle_waypoints, orbit_parity_probe,
			)
		if probe_path.size() < 2:
			...
			if probe_path.size() < 2:
				if not orbit_hover_paint:
					_clear_stale_painted_preview_route(p_unit.id)
					_refresh_click_target_highlight()
					_restore_parked_preview_board(parked_preview_board)
					return
		var leg_origin: Vector2i = _phase_entry_stand(p_unit)
		var leg_wps: Array[Vector2i] = []
		if not (orbit_hover_paint and probe_path.size() < 2):
			leg_wps = CombatPlanningPreview.destination_cells_from_route(
				probe_path, leg_origin, cell,
			)
		if orbit_hover_paint and probe_path.size() < 2:
			_apply_orbit_corridor_preview_path(p_unit, cell, probe_path)
			settle_waypoints = []
		elif not leg_wps.is_empty():
...
func _apply_assembler_prefix_preview_on_painted_route(p_unit: UnitState, cell: Vector2i) -> void:
	...
	if prefix_path.is_empty():
		return
	if prefix_path.size() < 2 and not (
		_voluntary_walk_orbit_settle_open(p_unit) and not dragging
	):
		return
	if prefix_path.size() >= 2:
		CombatPlanningPreview.set_unit_preview_path(preview_state, p_unit.id, prefix_path)
	elif prefix_path.size() == 1:
		CombatPlanningPreview.set_unit_stand_anchor_path(
			preview_state, p_unit.id, prefix_path[0] as Vector2i,
		)
	else:
...
func _apply_orbit_corridor_preview_path(
	...
	if orbit_path.is_empty():
		if _painted_receipt_orbit_extend_active(p_unit, cell):
			...
			if orbit_path.size() < 2:
				orbit_path = _assemble_voluntary_walk_preview_path(p_unit.id, p_unit, cell, [], true)
	...
	if orbit_path.is_empty():
		...
		return
	if orbit_path.size() >= 2:
		CombatPlanningPreview.set_unit_preview_path(preview_state, p_unit.id, orbit_path)
	elif orbit_path.size() == 1:
		CombatPlanningPreview.set_unit_stand_anchor_path(
			preview_state, p_unit.id, orbit_path[0] as Vector2i,
		)
	else:
		CombatPlanningPreview.clear_unit_preview_path(preview_state, p_unit.id)
	_sync_orbit_preview_paths_to_receipt(p_unit.id, orbit_path)
	_sync_movement_hover_paths_to_overlay(p_unit.id)


func _preview_path_for_unit(unit_id: int) -> Array:
	var route: Variant = preview_state.preview_paths.get(unit_id, null)
	if route is Array:
		return (route as Array).duplicate()
	return []


func _sync_orbit_preview_paths_to_receipt(unit_id: int, orbit_path: Array) -> void:
	if _settled_hover_preview == null or not _settled_hover_preview.valid:
		return
	if _settled_hover_preview.unit_id != unit_id:
		return
	var receipt_paths: Dictionary = _settled_hover_preview.preview_paths.duplicate(true)
	if orbit_path.is_empty():
		receipt_paths.erase(unit_id)
	elif orbit_path.size() >= 2:
		receipt_paths[unit_id] = orbit_path.duplicate()
	else:
		receipt_paths[unit_id] = [orbit_path[0]]
	_settled_hover_preview.preview_paths = receipt_paths
...
func _assemble_voluntary_walk_preview_path(
	...
	hover_cell: Vector2i,
	waypoints: Array[Vector2i],
	painted_premove_orbit_parity: bool = false,
) -> Array[Vector2i]:
	if actor == null or _director == null:
		return []
	var armed_painted_orbit_parity: bool = (
	...
	var leg_sealed: bool = (
		false
		if painted_premove_orbit_parity or armed_painted_orbit_parity
		else _assembler_treats_painted_leg_sealed(unit_id, actor, hover_cell)
	)
	...
	if (
		...
		if orbit_origin.x <= -900000:
			return []
		if hover_cell == orbit_origin:
			return [orbit_origin]
		if _director.board != null:
			var occ: UnitState = _director.board.get_unit_at(hover_cell)
			if occ != null and occ.is_enemy() and occ.id != actor.id:
				return [orbit_origin]
		if not _is_hover_move_cell(actor, hover_cell):
			return [orbit_origin]
		var orbit_move_legal: bool = _can_move_to(actor, hover_cell)
		if (
			_armed_awaiting_move_orbit_settle_open(actor)
			and _painted_preview_route_active(actor.id)
			and _hover_orbit_extends_painted_receipt(actor, hover_cell)
		):
			orbit_move_legal = true
		if not orbit_move_legal:
			return [orbit_origin]
		var receipt_corridor_extend: bool = (
			_hover_orbit_extends_painted_receipt(actor, hover_cell)
			and not painted_premove_orbit_parity
		)
		var orbit_board: BoardState = (
			_corridor_board_for_unarmed_premove_orbit(actor, orbit_origin)
			if receipt_corridor_extend
			else _corridor_board_for_voluntary_walk(actor)
		)
		var pathfind_actor: UnitState = actor
		if orbit_board != null:
			var board_actor: UnitState = orbit_board.get_unit_by_id(actor.id)
			if board_actor != null:
		...
		var orbit_corridor: Array[Vector2i] = CombatPlanningPreview.voluntary_walk_corridor_waypoints(
			...
			(
				...
				if receipt_corridor_extend
				else _move_budget(pathfind_actor)
			),
			_director,
		)
		if orbit_corridor.is_empty() or orbit_corridor.back() != hover_cell:
			return [orbit_origin]
		var orbit_path: Array[Vector2i] = [orbit_origin]
		for orbit_wp: Vector2i in orbit_corridor:
			orbit_path.append(orbit_wp)
...
func _resolve_commit_move_waypoints(unit_id: int, actor: UnitState, cell: Vector2i) -> Array[Vector2i]:
	...
	if _drag_route.size() >= 2:
		...
		if not drag_wps.is_empty() and drag_wps.back() == cell:
			return drag_wps
	var painted_path: Array = _authoritative_route_for_unit(unit_id)
	if painted_path.size() >= 2 and not _painted_receipt_orbit_extend_active(actor, cell):
		var leg: Array[Vector2i] = CombatPlanningPreview.destination_cells_from_route(
		...
		if not leg.is_empty() and leg.back() == cell:
			return leg
	var ability: AbilityData = _selected_ability_data(actor)
	if ability != null and not _painted_receipt_orbit_extend_active(actor, cell):
	...
	if painted_path.size() >= 2 and _painted_receipt_orbit_extend_active(actor, cell):
		var receipt_corridor: Array[Vector2i] = _corridor_waypoints_to_cell(
			actor,
			cell,
		)
		if not receipt_corridor.is_empty() and receipt_corridor.back() == cell:
		...
		if orbit_assembled.size() >= 2:
			var orbit_leg: Array[Vector2i] = CombatPlanningPreview.destination_cells_from_route(
				orbit_assembled, move_origin, cell,
			)
			if not orbit_leg.is_empty() and orbit_leg.back() == cell:
				return orbit_leg
...
## Receipt-orbit extend pathfinds on live turn board (premove parity — not sealed landing occupancy).
...
func _painted_preview_route_active(unit_id: int) -> bool:
	return CombatPlanningPreview.sealed_painted_geometry_route(
		preview_state, unit_id,
	).size() >= 2 or _authoritative_route_for_unit(unit_id).size() >= 2


func _painted_receipt_orbit_extend_active(p_unit: UnitState, cell: Vector2i) -> bool:
	if p_unit == null or not _painted_preview_route_active(p_unit.id):
		return false
...
func _receipt_orbit_corridor_preview_path(p_unit: UnitState, cell: Vector2i) -> Array[Vector2i]:
	return _painted_orbit_corridor_preview_path(p_unit, cell)


## Painted-route orbit corridor — armed MOVE-awaiting matches unarmed premove assembler path.
...
func _corridor_waypoints_to_cell(
	actor: UnitState,
	cell: Vector2i,
	force_unsealed_orbit_budget: bool = false,
) -> Array[Vector2i]:
	if actor == null or _director == null:
		return []
	if not _voluntary_walk_corridor_paint_active(actor):
	...
	if corridor_orbit_extend and origin.x > -900000:
		corridor_board = _corridor_board_for_unarmed_premove_orbit(actor, origin)
	var pathfind_actor: UnitState = actor
...
func action_range_visible_for_hover() -> bool:
	if dragging:
		if (
			...
			return false
		var drag_route: Array = _authoritative_route_for_unit(_director.selected_unit_id)
			...
			if awaiting_targeting_active() or has_awaiting_action:
				...
				if AbilitySystem.planning_is_valid_awaiting_endpoint(
					...
					if _painted_receipt_orbit_extend_active(actor, cell):
						var receipt_origin: Vector2i = _phase_entry_stand(actor)
						if receipt_origin.x > -900000:
							var receipt_corridor: Array[Vector2i] = (
								_corridor_waypoints_to_cell(
									actor,
									cell,
								)
							)
