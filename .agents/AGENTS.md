...
- **Push to remote every 3 commits:** After every 3rd local commit (count since the last push), run `git push origin main` to sync the remote. Also **always push at the end of a session** (i.e., when wrapping up work or before an IDE handoff). The push step must follow the commit in the same turn so the remote stays no more than 3 prompts behind local. If a push fails due to auth or no-remote errors, report it clearly rather than silently skipping.
