class_name EvasiveScenarioTest
extends RefCounted
...
## Bible: Evasive — Long move ROOT immune.
## Globals: MercenarySystems + AbilitySystem + Simulator.simulate_player_turn
## Data/Sim delegate: tests/harness/mercenary_qa_harness_scenarios.gd::run_evasive


static func run_all(failures: Array[String]) -> void:
	_sim_trigger(failures)
	_Planning.run_for_factory(failures, &"evasive")


static func _sim_trigger(failures: Array[String]) -> void:
	_Scenarios.run_evasive(failures)
	_sim_upgrade(failures)


static func _sim_upgrade(failures: Array[String]) -> void:
	_Upgrades.run_for_factory(failures, &"evasive")
