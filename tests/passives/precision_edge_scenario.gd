class_name PrecisionEdgeScenarioTest
extends RefCounted
...
## Bible: Precision Edge — Full HP bonus + bleed.
## Globals: MercenarySystems + AbilitySystem + Simulator.simulate_player_turn
## Data/Sim delegate: tests/harness/mercenary_qa_harness_scenarios.gd::run_precision_edge


static func run_all(failures: Array[String]) -> void:
	_sim_trigger(failures)
	_Planning.run_for_factory(failures, &"precision_edge")


static func _sim_trigger(failures: Array[String]) -> void:
	_Scenarios.run_precision_edge(failures)
	_sim_upgrade(failures)


static func _sim_upgrade(failures: Array[String]) -> void:
	_Upgrades.run_for_factory(failures, &"precision_edge")
