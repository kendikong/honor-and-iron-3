class_name TacticalVersatilityScenarioTest
extends RefCounted
...
## Bible: Tactical Versatility — Skill then basic buff.
## Globals: MercenarySystems + AbilitySystem + Simulator.simulate_player_turn
## Data/Sim delegate: tests/harness/mercenary_qa_harness_scenarios.gd::run_tactical_versatility


static func run_all(failures: Array[String]) -> void:
	_sim_trigger(failures)
	_Planning.run_for_factory(failures, &"tactical_versatility")


static func _sim_trigger(failures: Array[String]) -> void:
	_Scenarios.run_tactical_versatility(failures)
	_sim_upgrade(failures)


static func _sim_upgrade(failures: Array[String]) -> void:
	_Upgrades.run_for_factory(failures, &"tactical_versatility")
