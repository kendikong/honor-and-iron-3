...
## Bible: Fetch — RANGE 4 item/corpse to adjacent; [+] pull light allies 2 tiles.
## Globals: AbilitySystem targeting and PhysicsSystem pull.
...
static func run_all(failures: Array[String]) -> void:
	...
	_Planning.run_for_factory(failures, &"beast_fetch")

static func _data_contract(failures: Array[String]) -> void:
	_H.run_factory_matrix(failures)

static func _sim_contract(failures: Array[String]) -> void:
	_H.run_ability_row(&"beast_fetch", failures)

static func _sim_upgrade(failures: Array[String]) -> void:
	_H.run_ability_upgrade_row(&"beast_fetch", failures)
