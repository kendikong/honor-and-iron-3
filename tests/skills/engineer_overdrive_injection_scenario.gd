...
## Bible §12: Overdrive Injection — RANGE 1 +2 STR OVERCLOCK, 2 self damage; [+] refund 1 Scrap on death.
...
static func run_all(failures: Array[String]) -> void:
	...
	_Planning.run_for_factory(failures, &"engineer_overdrive_injection")

static func _data_contract(failures: Array[String]) -> void:
	_H.run_factory_matrix(failures)

static func _sim_contract(failures: Array[String]) -> void:
	_H.run_ability_row(&"engineer_overdrive_injection", failures)

static func _sim_upgrade(failures: Array[String]) -> void:
	_H.run_ability_upgrade_row(&"engineer_overdrive_injection", failures)
