...
## Bible §12: EMP Grenade — RANGE 4 AOE 2x2 PURGE SILENCE destroy constructs; [+] friendly HEAL 10 OVERCLOCK.
...
static func run_all(failures: Array[String]) -> void:
	...
	_Planning.run_for_factory(failures, &"engineer_emp_grenade")

static func _data_contract(failures: Array[String]) -> void:
	_H.run_factory_matrix(failures)

static func _sim_contract(failures: Array[String]) -> void:
	_H.run_ability_row(&"engineer_emp_grenade", failures)

static func _sim_upgrade(failures: Array[String]) -> void:
	_H.run_ability_upgrade_row(&"engineer_emp_grenade", failures)
