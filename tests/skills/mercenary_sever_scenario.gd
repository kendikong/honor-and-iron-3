class_name MercenarySeverScenarioTest
extends RefCounted
...
## Bible: Sever — On kill ally heal.
## Globals: MercenarySystems + AbilitySystem + Simulator.simulate_player_turn
## Data/Sim delegate: tests/harness/mercenary_qa_harness_scenarios.gd::run_sever


static func run_all(failures: Array[String]) -> void:
	_sim_contract(failures)
	_Planning.run_for_factory(failures, &"mercenary_sever")


static func _sim_contract(failures: Array[String]) -> void:
	_Scenarios.run_sever(failures)
	_sim_upgrade(failures)


static func _sim_upgrade(failures: Array[String]) -> void:
	_Upgrades.run_for_factory(failures, &"mercenary_sever")
