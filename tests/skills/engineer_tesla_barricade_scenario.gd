...
## Bible §12: Tesla Barricade — RANGE 1 build wall; [+] Manual Detonation applies STAGGER.
...
static func run_all(failures: Array[String]) -> void:
	...
	_Planning.run_for_factory(failures, &"engineer_tesla_barricade")

static func _data_contract(failures: Array[String]) -> void:
	_H.run_factory_matrix(failures)

static func _sim_contract(failures: Array[String]) -> void:
	_H.run_ability_row(&"engineer_tesla_barricade", failures)

static func _sim_upgrade(failures: Array[String]) -> void:
	_H.run_ability_upgrade_row(&"engineer_tesla_barricade", failures)
