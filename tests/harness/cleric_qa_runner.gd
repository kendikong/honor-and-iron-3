...
const _REGISTRY := preload("res://tests/harness/cleric_scenario_registry.gd")
const _HARNESS := preload("res://tests/harness/cleric_qa_harness.gd")
const _MOVEMENT_SMOKE := preload("res://tests/harness/movement_planning_smoke_registry.gd")


static func run_all(failures: Array[String]) -> void:
	_HARNESS.run_factory_shell(failures)
	for entry: Dictionary in _REGISTRY.all_entries():
		...
		print("[CLERIC_QA] %s (%s)" % [name, factory_id])
