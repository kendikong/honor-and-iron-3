...
static func run_for_factory(failures: Array[String], factory_id: StringName) -> void:
	...
	if ability != null:
		...
		match class_id:
			...
			"mercenary":
				_MercenaryUpgrades.run_upgrade_for(row_name, failures)
			_:
				pass
		return
	var merc_passive: PassiveData = _MercenaryHarness.factory_passive(factory_id)
	if merc_passive != null and not merc_passive.upgraded_description.is_empty():
		_MercenaryUpgrades.run_passive_upgrade_for(factory_id, failures)
