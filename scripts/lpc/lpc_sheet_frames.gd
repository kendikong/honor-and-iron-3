...
static func get_lazy_frames(path_prefix: String, recolor: String, variant: String) -> SpriteFrames:
	var cache_key = path_prefix + "|" + recolor + "|" + variant
...
static func ensure_animation(frames: SpriteFrames, action: StringName) -> void:
	...
	if base_action == "idle":
		_ensure_idle(frames, path_prefix, recolor, variant)
		return
	...
	var tex: Texture2D = _load_nearest_texture(sheet_path)
	if tex == null:
		return
...
static func _ensure_idle(frames: SpriteFrames, path_prefix: String, recolor: String, variant: String) -> void:
	...
	if FileAccess.file_exists(walk_path):
		var tex = _load_nearest_texture(walk_path)
		if tex != null:
			for dir_i in range(LpcConstants.DIRS.size()):
				var anim_name = StringName("idle_" + LpcConstants.DIRS[dir_i])
				frames.add_animation(anim_name)
...
static func _load_nearest_texture(path: String) -> Texture2D:
