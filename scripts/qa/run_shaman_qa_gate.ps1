...
$latestGateLog = Join-Path $projectRoot "reports\qa\qa_shaman_gate_latest.txt"
...
function Save-GateLog() {
	$canonical = Join-Path $projectRoot "reports\qa\qa_shaman_gate_canonical.txt"
	$gateLogLines | Set-Content -Path $canonical -Encoding utf8
	$tmp = Join-Path $projectRoot "reports\qa\qa_shaman_gate_latest.tmp"
...
$matrixDoc = Join-Path $projectRoot "docs\qa\classes\SHAMAN_QA_GATE.md"
$manifestPath = Join-Path $projectRoot "docs\qa\manifests\shaman_meta_critic_manifest.json"

Write-GateLine "=== Shaman QA gate (class validation - NOT planning QA) ==="
Write-GateLine "Spec: docs/qa/classes/SHAMAN_QA_GATE.md"
Write-GateLine ""
...
Write-GateLine "=== Matrix summary (from docs/qa/classes/SHAMAN_QA_GATE.md) ==="
...
if (Test-Path $manifestPath) {
	...
	Write-GateLine "[WARN] Missing manifest: docs/qa/manifests/shaman_meta_critic_manifest.json"
}
...
foreach ($entrypoint in $conversionEntrypoints) {
	...
	$conversionStdout = Join-Path $env:TEMP "honor-and-iron-shaman-conversion.stdout.log"
	$conversionStderr = Join-Path $env:TEMP "honor-and-iron-shaman-conversion.stderr.log"
...
$process = Start-Process -FilePath $GodotPath `
	-ArgumentList "--headless --path `"$projectRoot`" res://tests/gates/ShamanQaGate.tscn" `
