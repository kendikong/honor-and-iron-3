...
Write-GateLine "=== Engineer QA gate (CLASS_QA_BIBLE) ==="
Write-GateLine "Spec: docs/qa/CLASS_QA_BIBLE.md (instance: docs/qa/classes/ENGINEER_QA_GATE.md)"
Write-GateLine ""
...
Write-GateLine "=== Matrix summary (from docs/qa/classes/ENGINEER_QA_GATE.md) ==="
...
if (Test-Path $manifestPath) {
	...
	Write-GateLine "[WARN] Missing manifest: docs/qa/manifests/engineer_meta_critic_manifest.json"
}
...
$process = Start-Process -FilePath $GodotPath `
	-ArgumentList "--headless --path `"$projectRoot`" res://tests/gates/EngineerQaGate.tscn" `
...
Write-GateLine ""
Write-GateLine "=== Tier 2: live Engineer acceptance ==="
$liveScript = Join-Path $PSScriptRoot "run_engineer_live_qa.ps1"
& $liveScript -GodotPath ($GodotPath -replace '_console', '')
if ($LASTEXITCODE -ne 0) {
	Write-GateLine "[FAIL] Engineer Tier 2 live gate exit $LASTEXITCODE"
	Exit-Gate 4
}
Write-GateLine "[PASS] Engineer Tier 2 live preview/factory gate"

Write-GateLine ""
Write-GateLine "=== Engineer QA gate summary ==="
...
if ($passRows.Count -eq $requiredFactoryIds.Count) {
	Write-GateLine "[PASS] Engineer QA gate: matrix 31/31 + Tier 1 harness + Tier 2 live + AOE footprint PASS (automated bar; owner sign-off separate per CLASS_QA_SIGNOFF.md)."
	Exit-Gate 0
}
