...
$latestGateLog = Join-Path $projectRoot "reports\qa\qa_bruiser_gate_latest.txt"
...
function Save-GateLog() {
	$canonical = Join-Path $projectRoot "reports\qa\qa_bruiser_gate_canonical.txt"
	$gateLogLines | Set-Content -Path $canonical -Encoding utf8
	$tmp = Join-Path $projectRoot "reports\qa\qa_bruiser_gate_latest.tmp"
...
Write-GateLine "=== Matrix summary (from docs/qa/classes/BRUISER_QA_GATE.md) ==="
...
if (Test-Path $manifestPath) {
	...
	Write-GateLine "[WARN] Missing manifest: docs/qa/manifests/bruiser_meta_critic_manifest.json"
}
...
foreach ($typedContract in @(
	...
	$contractStderr = Join-Path $env:TEMP ("honor-and-iron-bruiser-$contractTag.stderr.log")
...
$process = Start-Process -FilePath $GodotPath `
	-ArgumentList "--headless --path `"$projectRoot`" res://tests/gates/BruiserQaGate.tscn" `
	-RedirectStandardOutput $stdoutPath `
	-RedirectStandardError $stderrPath `
	-PassThru
$exitCode = Wait-GodotProcessWithEscCancel -Process $process -Label "Bruiser Tier 1 harness"
if ($exitCode -eq 130) {
	Write-GateLine "[CANCEL] Bruiser Tier 1 harness stopped by ESC."
	Exit-Gate 130
}
