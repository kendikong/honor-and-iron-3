...
$latestGateLog = Join-Path $projectRoot "reports\qa\qa_mercenary_gate_latest.txt"
...
function Save-GateLog() {
	$canonical = Join-Path $projectRoot "reports\qa\qa_mercenary_gate_canonical.txt"
	$gateLogLines | Set-Content -Path $canonical -Encoding utf8
	$tmp = Join-Path $projectRoot "reports\qa\qa_mercenary_gate_latest.tmp"
...
Write-GateLine "Spec: docs/qa/classes/MERCENARY_QA_GATE.md"
Write-GateLine ""
Write-GateLine "=== Matrix summary (from docs/qa/classes/MERCENARY_QA_GATE.md) ==="
...
if (Test-Path $manifestPath) {
	...
	Write-GateLine "[WARN] Missing manifest: docs/qa/manifests/mercenary_meta_critic_manifest.json"
}
...
$process = Start-Process -FilePath $GodotPath `
	-ArgumentList "--headless --path `"$projectRoot`" res://tests/gates/MercenaryQaGate.tscn" `
