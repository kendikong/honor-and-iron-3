...
$latestGateLog = Join-Path $projectRoot "reports\qa\qa_monk_gate_latest.txt"
...
function Save-GateLog() {
	$canonical = Join-Path $projectRoot "reports\qa\qa_monk_gate_canonical.txt"
	$gateLogLines | Set-Content -Path $canonical -Encoding utf8
	$tmp = Join-Path $projectRoot "reports\qa\qa_monk_gate_latest.tmp"
...
$matrixDoc = Join-Path $projectRoot "docs\qa\classes\MONK_QA_GATE.md"
$manifestPath = Join-Path $projectRoot "docs\qa\manifests\monk_meta_critic_manifest.json"

Write-GateLine "=== Monk QA gate (class validation - NOT planning QA) ==="
Write-GateLine "Spec: docs/qa/classes/MONK_QA_GATE.md"
Write-GateLine ""
...
Write-GateLine "=== Matrix summary (from docs/qa/classes/MONK_QA_GATE.md) ==="
...
if (Test-Path $manifestPath) {
	...
	Write-GateLine "[WARN] Missing manifest: docs/qa/manifests/monk_meta_critic_manifest.json"
}
...
foreach ($typedContract in @(
	...
	$contractStdout = Join-Path $env:TEMP ("honor-and-iron-monk-$contractTag.stdout.log")
	$contractStderr = Join-Path $env:TEMP ("honor-and-iron-monk-$contractTag.stderr.log")
...
$process = Start-Process -FilePath $GodotPath `
	-ArgumentList "--headless --path `"$projectRoot`" res://tests/gates/MonkQaGate.tscn" `
