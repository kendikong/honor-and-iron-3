...
$latestGateLog = Join-Path $projectRoot "reports\qa\qa_archer_gate_latest.txt"
...
function Save-GateLog() {
	$canonical = Join-Path $projectRoot "reports\qa\qa_archer_gate_canonical.txt"
	$gateLogLines | Set-Content -Path $canonical -Encoding utf8
	$tmp = Join-Path $projectRoot "reports\qa\qa_archer_gate_latest.tmp"
...
Write-GateLine "Spec: docs/qa/classes/ARCHER_QA_GATE.md"
Write-GateLine ""
...
Write-GateLine "=== Matrix summary (from docs/qa/classes/ARCHER_QA_GATE.md) ==="
...
if (Test-Path $manifestPath) {
	...
	Write-GateLine "[WARN] Missing manifest: docs/qa/manifests/archer_meta_critic_manifest.json"
}
...
foreach ($typedContract in @(
	...
	$contractStderr = Join-Path $env:TEMP ("honor-and-iron-archer-$contractTag.stderr.log")
...
$process = Start-Process -FilePath $GodotPath `
	-ArgumentList "--headless --path `"$projectRoot`" res://tests/gates/ArcherQaGate.tscn" `
