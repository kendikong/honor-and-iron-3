...
if (-not (Test-Path $GodotPath)) {
	Write-Output "[INCOMPLETE] Beast Rider live QA: Godot executable not found: $GodotPath"
	exit 2
}
...
if ($process.ExitCode -ne 0) {
	Write-Output "[FAIL] Beast Rider live QA exit $($process.ExitCode)"
	exit 1
}
Write-Output "[PASS] Beast Rider live QA"
exit 0
