	...
	if need_geometry:
		...
		if actor != null:
			...
			_foot_bake_cache[actor_id] = {
				"image": img.duplicate(),
				"bake_sig": bake_sig,
				"gpu_tex": ImageTexture.create_from_image(img),
			}
...
static func sync_unit_feet_gpu(
	...
	if settings == null or not settings.oblique_contact_shadows:
		...
		_sync_ground_shadow_uniforms(settings, shadow_root)
		return
	var layout_key: int = _unit_feet_gpu_layout_hash(actors)
	...
	_refresh_unit_feet_gpu_slots(actors, shadow_root)
...
static func _foot_bake_image_from_cache(actor_id: int) -> Image:
	var cached: Variant = _foot_bake_cache.get(actor_id)
	if cached is Dictionary:
		return (cached as Dictionary).get("image") as Image
	if cached is Image:
...
static func _build_unit_foot_gpu_slot(actor: Node2D, shadow_root: Node2D) -> Dictionary:
	...
	if sprite == null or sprite.texture == null:
		return {}
	var actor_id: int = actor.get_instance_id()
	var image: Image = _foot_bake_image_from_cache(actor_id)
...
static func _unit_feet_gpu_layout_hash(actors: Array) -> int:
	var parts: Array = []
	for actor_var: Variant in actors:
		var actor: Node2D = actor_var as Node2D
		if actor == null or not is_instance_valid(actor):
		...
		var bake_sig: int = -1
		if cached is Dictionary:
			bake_sig = int((cached as Dictionary).get("bake_sig", -1))
		parts.append([
			actor_id,
			bake_sig,
			image.get_width(),
...
static func _refresh_unit_feet_gpu_slots(actors: Array, shadow_root: Node2D) -> void:
	if _unit_feet_gpu_slots.is_empty() or shadow_root == null:
		return
...
static func _push_unit_feet_gpu_uniforms(mat: ShaderMaterial) -> void:
	...
	for i: int in range(MAX_UNIT_FEET_GPU):
		if i < count:
			...
			rects[i] = slot.get("map_rect", Vector4.ZERO) as Vector4
